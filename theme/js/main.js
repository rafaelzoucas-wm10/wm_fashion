(($) => {
    ...
    window.theme = {
            ...
            setTimeout(() => {
                $('#form-comments .submit-review').on('click', function (e) {
                    if (!$('#form-comments .stars .starn.icon-star').length) {
                        const textError = 'Avalia��o do produto obrigat�ria, d� sua avalia��o por favor';
        ...
        organizePagesTray: function () {
            ...
            titleReviewPage.prepend(
                '<button class="review-form" data-toggle="reviews">Deixei seu depoimento sobre n�s <span class="icon-arrow-simple" aria-hidden="true"></span></button>'
            );
        ...
        updateCartTotal: function () {
            const cartQuantity = $('.cart-quantity[data-cart="amount"]');
            const cartQuantityText = cartQuantity.text();
            
            cartQuantity.text($('.cart-preview-item').length);
            
            if(cartQuantityText != "0") {
                cartQuantity.css("display", "inline-block");
            }
        },
    };
    ...
    $(() => {
        ...
        if ($('html').hasClass('page-home')) {
            ...
            theme.insertBreadcrumbNavigationInPage('Sobre n�s', true);
        } else if (
